(function() {
  var BALANCED_PAIRS, EXPLICIT_CALL, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_BLOCK, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, SINGLE_CLOSERS, SINGLE_LINERS, left, rite, _i, _len, _ref;
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.addImplicitParentheses = function() {
      ...
      return this.scanTokens(function(token, i, tokens) {
        var callObject, current, exist, explicit, next, prev, seenControl, seenSingle, tag, _ref, _ref2, _ref3, _ref4, _ref5;
        tag = token[0];
        if (tag === 'CLASS' || tag === 'IF') noCall = true;
        _ref = tokens.slice(i - 1, (i + 1 + 1) || 9e9), prev = _ref[0], current = _ref[1], next = _ref[2];
        callObject = !noCall && tag === 'INDENT' && next && next.generated && next[0] === '{' && prev && (_ref2 = prev[0], __indexOf.call(IMPLICIT_FUNC, _ref2) >= 0);
        explicit = (_ref3 = prev != null ? prev[0] : void 0, __indexOf.call(EXPLICIT_CALL, _ref3) >= 0);
        seenSingle = false;
        seenControl = false;
        if (__indexOf.call(LINEBREAKS, tag) >= 0) noCall = false;
        if (prev && !prev.spaced && tag === '?') token.call = true;
        if (token.fromThen) return 1;
        if (!(callObject || explicit || (prev != null ? prev.spaced : void 0) && (prev.call || (_ref4 = prev[0], __indexOf.call(IMPLICIT_FUNC, _ref4) >= 0)) && (__indexOf.call(IMPLICIT_CALL, tag) >= 0 || !(token.spaced || token.newLine) && __indexOf.call(IMPLICIT_UNSPACED_CALL, tag) >= 0))) {
          return 1;
        }
        ...
        this.detectEnd(i + 1, function(token, i) {
          var post, _ref5, _ref6;
          tag = token[0];
          if (explicit && (__indexOf.call(IMPLICIT_END, tag) >= 0 || __indexOf.call(EXPLICIT_CALL, tag) >= 0)) {
            return true;
          }
          ...
          return !token.generated && this.tag(i - 1) !== ',' && (__indexOf.call(IMPLICIT_END, tag) >= 0 || (tag === 'INDENT' && !seenControl)) && (tag !== 'INDENT' || (this.tag(i - 2) !== 'CLASS' && (_ref5 = this.tag(i - 1), __indexOf.call(IMPLICIT_BLOCK, _ref5) < 0) && (_ref6 = this.tag(i - 2), __indexOf.call(EXPLICIT_CALL, _ref6) < 0) && !((post = this.tokens[i + 1]) && post.generated && post[0] === '{')));
        }, action);
        exist = tokens[i - ((_ref5 = prev[0], __indexOf.call(EXPLICIT_CALL, _ref5) >= 0) ? 2 : 1)];
        if ((exist != null ? exist[0] : void 0) === '?') exist[0] = 'FUNC_EXIST';
        return 2;
      });
    };
    ...
    Rewriter.prototype.tagPostfixConditionals = function() {
      ...
      return this.scanTokens(function(token, i) {
        ...
        this.detectEnd(i + 1, condition, function(token, i) {
          var _ref;
          if (token[0] !== 'INDENT' || (_ref = this.tokens[i - 1][0], __indexOf.call(EXPLICIT_CALL, _ref) >= 0)) {
