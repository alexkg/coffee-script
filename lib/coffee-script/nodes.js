(function() {
  var Access, Arr, Assign, Base, Block, Call, Class, Closure, Code, Comment, Existence, Extends, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, Op, Package, Param, Parens, Push, RESERVED, Range, Return, SIMPLENUM, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, compact, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref;
  ...
  exports.Package = Package = (function() {
    __extends(Package, Base);
    function Package(variable, body) {
      this.variable = variable;
      this.body = body != null ? body : new Block;
      this.body.packageBody = true;
    }
    Package.prototype.children = ['variable', 'body'];
    Package.prototype.isAssignable = NO;
    Package.prototype.determineName = function() {
      var decl, tail;
      if (!this.variable) return null;
      decl = (tail = last(this.variable.properties)) ? tail instanceof Access && tail.name.value : this.variable.base.value;
      return decl && (decl = IDENTIFIER.test(decl) && decl);
    };
    Package.prototype.setContext = function(name) {
      return this.body.traverseChildren(false, function(node) {
        var p, ps, v, _i, _len, _ref2;
        if (node.classBody || node.packageBody) return false;
        if (node instanceof Package || node instanceof Class) {
          if (v = node.variable) {
            ps = [];
            ...
            return node.variable = new Value(new Literal(name), ps);
          }
        } else if (node instanceof Literal && node.value === 'this') {
          return node.value = name;
        } else if (node instanceof Code) {
          node.klass = name;
          if (node.bound) return node.context = name;
    ...
    Package.prototype.addProperties = function(node, name, o) {
      var assign, base, exprs, func, props;
      props = node.base.properties.slice(0);
      exprs = (function() {
        var _results;
    ...
    Package.prototype.walkBody = function(name, o) {
      return this.traverseChildren(false, __bind(function(child) {
        var exps, i, node, _len, _ref2;
        if (child instanceof Package || child instanceof Class) return false;
        if (child instanceof Block) {
          _ref2 = exps = child.expressions;
          for (i = 0, _len = _ref2.length; i < _len; i++) {
            node = _ref2[i];
    ...
    Package.prototype.compileNode = function(o) {
      var access, line, name, nameLiteral, nameValue, pkg, pre, scopeArguments, scopeParameters, seed, _i, _len, _ref2;
      name = this.determineName();
      if (!(name != null) || name === 'this') name = '_Package';
      nameLiteral = new Literal(name);
      nameValue = new Value(nameLiteral);
      this.setContext(name);
      this.walkBody(name, o);
      this.body.push(nameLiteral);
      scopeParameters = [];
      scopeArguments = [];
      pre = [];
      if (this.variable && this.variable.properties.length) {
        seed = this.variable.base;
        ...
        scopeArguments.push(this.variable ? this.variable : new Obj);
      }
      pre = Block.wrap(pre);
      pkg = new Code(scopeParameters, this.body);
      pkg = new Parens(pkg);
