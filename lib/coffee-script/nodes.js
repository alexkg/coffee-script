(function() {
  ...
  exports.Value = Value = (function() {
    ...
    Value.prototype.isAccess = function() {
      return last(this.properties) instanceof Access;
    };
    Value.prototype.getReceiver = function() {
      return new Value(this.base, this.properties.slice(0, -1));
    };
  ...
  exports.Call = Call = (function() {
    __extends(Call, Base);
    function Call(variable, args, soak, tag) {
      ...
      if (tag) this[tag] = true;
      if (this.curry) {
        if (this.isNew || this.isSuper) {
          throw SyntaxError("can't curry `new` or `super` calls");
        }
        if (!(args.length || variable instanceof Value && variable.isAccess())) {
          throw SyntaxError("no args to curry and no receiver to bind");
        }
      }
    }
    Call.prototype.children = ['variable', 'args'];
    Call.prototype.toString = function(idt) {
      return Call.__super__.toString.call(this, idt, this.constructor.name + (this.curry ? '~' : void 0));
    };
    ...
    Call.prototype.compileNode = function(o) {
      ...
      if (this.isSuper) {
        ...
        fun = this.variable.compile(o, LEVEL_ACCESS);
        if (!this.curry) {
          return (this.isNew ? 'new ' : '') + fun + ("(" + args + ")");
        }
        if (this.variable instanceof Value && this.variable.isAccess()) {
          fun = utility('bind') + ("(" + fun + ", " + (this.variable.getReceiver().compile(o)) + ")");
        }
        if (args.length) {
    ...
    Call.prototype.compileSplat = function(o, splatArgs) {
      ...
      if (!this.curry) return "" + fun + ".apply(" + ref + ", " + splatArgs + ")";
      if (name) fun = utility('bind') + ("(" + fun + ", " + ref + ")");
      return utility('curry') + (".apply(" + fun + ", " + splatArgs + ")");
    };
    return Call;
  })();
