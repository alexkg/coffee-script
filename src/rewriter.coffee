...
class exports.Rewriter
  ...
  addImplicitParentheses: ->
    ...
    condition = (token, i) ->
      ...
      return yes if prev and prev[0] in EXPLICIT_CALL and tag in IMPLICIT_END
    ...
    @scanTokens (token, i, tokens) ->
      ...
      wasImplicit = prev and prev[0] in IMPLICIT_FUNC
      wasExplicit = prev and prev[0] in EXPLICIT_CALL
      callObject  = not noCall and tag is 'INDENT' and wasImplicit and
                    next and next.generated and next[0] is '{'
      seenSingle  = no
      seenControl = no
      noCall      = no if tag in LINEBREAKS
      token.call  = yes if prev and not prev.spaced and tag is '?'
      return 1 if token.fromThen
      return 1 unless callObject or wasExplicit or
        prev?.spaced and (prev.call or wasImplicit) and
        (tag in IMPLICIT_CALL or not (token.spaced or token.newLine) and tag in IMPLICIT_UNSPACED_CALL)
      tokens.splice i, 0, @generate 'CALL_START', '(', token[2]
      if wasExplicit and tag is ',' # allows parameterless <~ in argument list
        action.call @, next, i + 1
        return 2
      @detectEnd i + 1, condition, action
      prev = tokens[i - 2] if wasExplicit
